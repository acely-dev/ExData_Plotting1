#load library
library(lubridate)


#reading subset dataset
df_subset <- read.csv("data_subset.csv")
df_subset$date_form <- ymd_hms(df_subset$date_form)

#plot 4
#set grid
par(mfrow=c(2,2))

with(df_subset, plot(date_form,Global_active_power, type="l",
                     ylab = "Global Activate Power",
                     xlab=""))

with(df_subset, plot(date_form,Voltage, type="l",
                     ylab = "Voltaje",
                     xlab="datatime"))

with(df_subset, plot(date_form,Sub_metering_1, type="l", xlab = "", 
                     ylab="Energy sub metering"), type="n")
with(df_subset, points(date_form,Sub_metering_2, type = "l", col="red"))
with(df_subset, points(date_form,Sub_metering_3, type = "l", col="blue"))
legend("topright", lty = 1, cex=c(0.2,0.2) ,col=c("black","blue","red"), 
       legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"))

with(df_subset, plot(date_form,Global_reactive_power, type="l",
                     ylab = "Global_reactive_power",
                     xlab="datatime"))
#copy plot to png file
dev.copy(png,file="plot4.png")
dev.off()

